...
class CardVisibility {
    constructor(showHands, openHands, chimera) {
        ...
        this.rules = [
            ...
            (card, player) => this.isChimeraGameRule(card, player)
        ];
    }
    ...
    isChimeraGameRule(card, player) {
        return (
            this.chimera && ['hand', 'archives'].includes(card.location)
        );
    }
