...
class ResolveStatusAbilityAction extends CardAction {
    ...
    setup() {
        ...
        if (this.ability) {
            this.effectMsg = `resolve {0}'s ${this.ability.title ? this.ability.title + ' ' : ''
                }ability`;
        } else {
        ...
        return super.createEvent('onAction', { card: card, context: context }, () => {
            ...
            context.game.queueSimpleStep(() => {
                if (ability && (!ability.condition || ability.condition(context))) {
                    ...
                    context.game.queueStep(new AbilityResolver(context.game, newContext));
                }
            });
        });
    }
...
module.exports = ResolveStatusAbilityAction;
