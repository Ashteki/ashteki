describe('Crystal Armor', function () {
    describe('with divine and time die', function () {
        beforeEach(function () {
            this.setupTest({
                player1: {
    ...
    describe('with no divine die but time die', function () {
        beforeEach(function () {
            this.setupTest({
                player1: {
                    phoenixborn: 'aradel-summergaard',
    ...
    describe('with divine die but no time die', function () {
        beforeEach(function () {
            this.setupTest({
                player1: {
                    phoenixborn: 'aradel-summergaard',
        ...
        it('no reaction prompt on unit attack', function () {
            this.player1.clickPrompt('Attack');
            this.player1.clickCard(this.fluteMage); // target
            this.player1.clickCard(this.mistSpirit); // single attacker

            expect(this.player2).not.toHavePrompt('Any reactions to attackers being declared?');
            expect(this.player2).not.toBeAbleToSelect(this.crystalArmor);

            expect(this.crystalArmor.location).toBe('hand');
            expect(this.player2.dicepool[0].exhausted).toBe(false);
        });
    });
});
