describe('Blood Shaman with undying heart', function () {
    beforeEach(function () {
        this.setupTest({
            ...
            player2: {
                ...
                dicepool: ['ceremonial', 'natural', 'natural', 'illusion', 'charm', 'charm']
            }
        });
    ...
    it('triggers both abilities when destroyed by own spell', function () {
        ...
        expect(this.player1.dicepool[0].exhausted).toBe(false);
        expect(this.player1).toBeAbleToSelectDie(0);
        this.player1.clickDie(0);

        this.player1.clickPrompt('Yes');

        expect(this.aradelSummergaard.damage).toBe(0);
        expect(this.player1.dicepool[0].level).toBe('class'); // blood shaman effect
